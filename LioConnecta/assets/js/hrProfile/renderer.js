...
function renderPageShell({ title, provider, isSimulated, bodyHtml }) {
  return `
    <section class="card hr-profile-page">
      <div class="card-header hr-profile-page__header">
        <div class="hr-profile-page__intro">
          <a href="#inicio" class="hr-profile-page__back">← Voltar ao inicio</a>
          <h2>${escapeHtml(title || "Perfil RH")}</h2>
        </div>
        <span class="panel-pill panel-pill--info">${escapeHtml(provider || "TOTVS RM")}${isSimulated ? " • Simulado" : ""}</span>
      </div>
      <div class="hr-profile-page__body">
        ${bodyHtml}
      </div>
    </section>
  `;
  ...
  return renderPageShell({
    ...
    bodyHtml: `
      ...
      <div class="hr-profile-section">
        <div class="hr-profile-section__head">
          <h3>Solicitacoes recentes</h3>
          ${data.canRequest ? `<button type="button" class="feed-composer-submit" disabled title="Integracao futura com TOTVS RM">Solicitar ferias</button>` : ""}
        </div>
        ${requests.length ? `
...
function renderPayslipPage(data = {}) {
  ...
  return renderPageShell({
    ...
    bodyHtml: `
      <div class="hr-profile-section">
        <div class="hr-profile-section__head">
          <h3>Comprovantes disponiveis</h3>
        </div>
        ${items.length ? `
    ...
    bodyHtml: `
      ...
      ${data.managerFeedback ? `
        <div class="hr-profile-section">
          ...
          <p class="hr-profile-feedback">${escapeHtml(data.managerFeedback)}</p>
        </div>
      ` : ""}
    `
  });
